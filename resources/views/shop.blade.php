...
<main class="pt-40">
               ...
               <ul class="list list-inline mb-0 brand-list">
                ...
                <li class="list-items">
                  <span class="menu-link py-1">
                    ...
                    value="{{$brand->id}}" @if (in_array($brand->id, explode(',', $f_brands ?? ''))) checked="checked" @endif>
                    {{$brand->name}}
                  </span>
      ...
      <div class="shop-list flex-grow-1">
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          <div class="breadcrumb mb-0 d-none d-md-block flex-grow-1">
            <a href="{{route('admin.index')}}" class="menu-link menu-link_us-s text-uppercase fw-medium">Home</a>
    ...
    <div class="swiper-slide">
        <a href="{{route('shop.product.details',['product_slug'=>$product->slug])}}">
            <img loading="lazy" src="{{ asset('uploads/productimage/thumbnails/' . trim($gimg)) }}"
                 width="330" height="400" alt="Gallery Image" class="pc__img">
        </a>
    </div>
@endforeach
                  </div>
            ...
            @else
              ...
              <div class="pc__info position-relative">
                ...
                <div class="product-card__price d-flex">
                  <span class="money price">@if($product->sale_price) <s>${{$product->regular_price}}</s>${{$product->sale_price}}
                @else
                ...
                <form method="POST" action="{{ route('wishlist.remove', ['id' => $product->id]) }}">
                    @csrf
                    <button type="submit" class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0" title="Remove from Wishlist" style="color: red;">
            ...
            @else
                ...
                <form method="POST" action="{{ route('wishlist.add') }}">
                    ...
                    <input type="hidden" name="price" value="{{ $product->sale_price ?: $product->regular_price }}" />
                    <button type="submit" class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0" title="Add To Wishlist" style="color: black;">
                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <use href="#icon_heart" />
                        </svg>
                    </button>
  ...
  <form method="GET" id="frmfilter" action="{{ route('shop.index') }}">
    ...
    <input type="hidden" name="brands" id="brands" value="{{ $f_brands }}">
</form>
@endsection
